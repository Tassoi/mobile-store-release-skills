...
android {
    namespace = "com.example.releasefixture"
    compileSdk = 35

    defaultConfig {
        applicationId = "com.example.releasefixture"
        minSdk = 24
        targetSdk = 35
        versionCode = 42
        versionName = "2.4.0"
    ...
    if (buildsReleaseArtifact && !hasValidKeystore) {
        throw GradleException(
            "Release signing is not configured. Add android/key.properties with " +
                "keyAlias, keyPassword, storeFile, and storePassword, and ensure " +
                "storeFile points to an existing keystore."
        )
    }
}
